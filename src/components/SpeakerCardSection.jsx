    ...
    >
      <div className="max-w-7xl mx-auto w-full relative z-10 space-y-8">
        ...
        <div className="flex flex-col sm:flex-row sm:items-end justify-between gap-4 border-b border-[#222222] pb-6">
          <div className="space-y-2">
            <div className="flex items-center gap-3">
              <span className="inline-flex items-center gap-1.5 px-3 py-1 rounded-full border border-[#FF2A1A]/40 bg-[#161616] text-[11px] font-mono tracking-widest uppercase text-[#FF2A1A] font-bold">
          ...
          <div className="flex items-center gap-1.5 bg-[#141414] border border-[#262626] p-1.5 rounded-xl self-start sm:self-auto overflow-x-auto max-w-full scrollbar-none">
            {[
              ...
              <button
                ...
                className={`px-3 sm:px-4 py-1.5 rounded-lg font-mono text-[11px] sm:text-xs tracking-wider transition-all whitespace-nowrap cursor-pointer ${
                  activeTab === tab.id
                    ? 'bg-[#FF2A1A] text-white font-bold shadow-md shadow-red-900/40'
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 items-stretch">
          ...
          <div className="lg:col-span-5 relative flex flex-col items-center justify-center p-3 sm:p-6 rounded-3xl overflow-hidden shadow-2xl border border-[#2B0E0E] bg-gradient-to-br from-[#2D0005] via-[#4D000B] to-[#120003] text-white">
          ...
          <div className="lg:col-span-7 flex flex-col justify-between bg-[#111111] border border-[#262626] rounded-3xl p-6 sm:p-8 text-white shadow-xl">
            ...
            {activeTab === 'clues' && (
              ...
              >
                <div className="space-y-1">
                  <span className="font-mono text-xs tracking-widest text-neutral-400 uppercase font-bold">
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                  {speaker.hints.map((hint, idx) => (
                    <div 
                      key={idx}
                      className="p-3.5 bg-[#181818] hover:bg-[#202020] border border-[#2A2A2A] rounded-xl transition-all shadow-xs"
                    >
                      ...
                      <p className="font-sans text-[11px] text-neutral-300 leading-relaxed pl-6">
                        {hint.detail}
                      </p>
                    </div>
                  ))}
                ...
                <div className="pt-2 flex flex-col sm:flex-row sm:items-center justify-between gap-3">
                  ...
                  <button
                    ...
                    className="inline-flex items-center justify-center gap-2 px-5 py-2.5 rounded-full font-mono text-xs font-bold tracking-wider bg-[#FF2A1A] hover:bg-[#D91C1C] text-white shadow-md transition-transform hover:scale-105 cursor-pointer"
                  >
                  ...
                  {speaker.dossier.map((d, i) => (
                    <div key={i} className="p-4 bg-[#181818] border border-[#2A2A2A] rounded-xl space-y-2">
                      <div className="flex items-center justify-between">
                        <span className="font-mono text-[10px] tracking-widest px-2 py-0.5 rounded bg-[#FF2A1A]/15 border border-[#FF2A1A]/40 text-[#FF2A1A] font-bold">
                          {d.tag}
                        </span>
                      ...
                      <p className="font-sans text-xs text-neutral-300">{d.desc}</p>
            ...
            {activeTab === 'guess' && (
              ...
              >
                <div className="space-y-2">
                  <div className="inline-flex items-center gap-2 px-3 py-1 rounded-full bg-[#181818] border border-[#FF2A1A]/40 text-[#FF2A1A] font-mono text-xs font-bold tracking-widest uppercase">
                    <Sparkles className="w-3.5 h-3.5" />
                    <span>DECODE SPEAKER IDENTITY</span>
                  </div>
                  ...
                  <p className="font-sans text-sm sm:text-base text-neutral-300 leading-relaxed">
                    Enter their name or associated company below to test your deduction.
                  </p>
                </div>
                ...
                <form onSubmit={handleGuessSubmit} className="space-y-4">
                  <div className="relative">
                    <input 
                      ...
                      className="w-full pl-5 pr-32 sm:pr-36 py-4 bg-black border border-[#333333] focus:border-[#FF2A1A] rounded-2xl text-sm sm:text-base font-sans font-medium text-white placeholder-neutral-500 focus:outline-none transition-all shadow-xs"
                    />
                    <button
                      type="submit"
                      className="absolute right-2 top-1/2 -translate-y-1/2 px-5 sm:px-6 py-2.5 sm:py-3 rounded-xl text-xs sm:text-sm font-display font-black tracking-wider uppercase bg-[#FF2A1A] hover:bg-[#D91C1C] text-white shadow-md transition-all hover:scale-105 active:scale-95 cursor-pointer"
                    >
                      SUBMIT
                    </button>
                  </div>
                ...
                {guessFeedback && (
                  ...
                  >
                    {guessFeedback.type === 'success' ? (
                      ...
                      <AlertCircle className="w-6 h-6 text-[#FF2A1A] shrink-0 mt-0.5" />
                    )}
                    <div className="space-y-0.5">
                      ...
                      <p className="font-sans text-sm font-medium text-neutral-200 leading-relaxed">{guessFeedback.message}</p>
                    </div>
                  </motion.div>
                )}

                {/* Quick Hint Cheatsheet */}
                <div className="p-3.5 bg-[#181818] border border-[#2A2A2A] rounded-xl text-xs font-mono text-neutral-400 flex items-center justify-between">
                  ...
                  <button 
                    ...
                    className="text-[#FF2A1A] font-bold hover:underline cursor-pointer"
                  >
                    Review Sticky Clues →
                  </button>
                </div>
            ...
            {activeTab === 'reveal' && (
              ...
              >
                ...
                {/* Quote */}
                <div className="p-4 bg-[#181818] border-l-4 border-[#FF2A1A] rounded-r-xl italic font-serif text-sm sm:text-base text-neutral-200">
                  "{speaker.revealed.quote}"
                </div>
            ...
            <div className="pt-6 border-t border-[#262626] flex items-center justify-between">
              ...
              {!isLast ? (
                <button
                  ...
                  className="inline-flex items-center gap-2 font-mono text-xs tracking-wider text-white hover:text-[#FF2A1A] transition-colors group cursor-pointer"
                >
                  <span className="font-bold">SCROLL TO NEXT SPEAKER</span>
                  <ArrowDown className="w-3.5 h-3.5 group-hover:translate-y-1 transition-transform text-[#FF2A1A]" />
                </button>
              ) : (
                <button
                  onClick={() => {
                  ...
                  className="inline-flex items-center gap-2 font-mono text-xs tracking-wider text-[#FF2A1A] font-bold hover:underline cursor-pointer"
                >
